...
class CLINChooses_or_Recs(PromptFormat):
    ...
    def format_prompt(self, history, subject, problem, valid_subjects, valid_topics, valid_causes, choices, summary,
        ...
        return [
            ...
            {
                "role": "user",
                "content": f"Task: Find the cause behind the {subject}'s {problem}\nYour previous experience with similar tasks:\n{formated_prev_exp}{summary_prompt}{summary}Interaction History:\n{formated_history}Student top choices:\n{formated_choices} available actions:{available_actions}\n{available_options} {question} {user_em}example output1:\n1, 3\n***\nchoose\n$$$\nreason: we need to explore the baby's symptoms more.\n###\n1\nexample output2:\n1, 3\n***\nrecommend\n$$$\nreason: we need to explore the baby's symptoms more but the student have not included it in their choice.\n###\n{output}"}
        ]
...
class CLINRecs_with_fallback(PromptFormat):
    ...
    def format_prompt(self, history, subject, problem, valid_subjects, valid_topics, valid_causes, previous_suggestions,
        ...
        available_options = f"valid causes: {formated_valid_causes}" if posttest else f"valid subjects: {formated_valid_subjects}\nvalid topics: {formated_valid_topics}\n"
        output = "1. choose(cause1)\n2. choose(cause2)" if posttest else "1. ask(x,y)\n2. answer()"
        reason = "Given the information received from the customer, this is the most probable cause." if posttest else "The customer's age is important in determining the cause of the problem."
        ...
        prompt = [
            ...
            {
                "role": "user",
                "content": f"Task: Find the cause behind the {subject}'s {problem}\nYour previous experience with similar tasks:\n{formated_prev_exp}{summary_prompt}{summary}Interaction History:\n{formated_history}available actions:{available_actions}\n{available_options}What are your top {2 if posttest else self.num_of_recs} recommended actions from the action list? you can first reason about your recommendation in less than 50 words. Don't forget to put ### after your reasoning finishes. Then write your suggested actions. {user_em}\nexample output:\n1, 3\n$$$\nreason: r\n###\n1"
            }
        ]
...
class GPTPlays(PromptFormat):
    ...
    def format_prompt(self, history, subject, problem, valid_subjects, valid_topics, valid_causes, posttest=False,
        ...
        available_actions = "1.choose(cause)[The most probable reason is cause]" if posttest else " 1. ask(subject,topic)[I want to ask about subject’s topic.] 2. answer()[I want to suggest the cause behind the customer’s problem.]"
        available_options = f"valid causes: {formated_valid_causes}" if posttest else f"valid subjects: {formated_valid_subjects}\nvalid topics: {formated_valid_topics}\n"
        reason = "Given the information received from the patient, this is the most probable cause." if posttest else "The patient's age is important in determining the cause of the problem."
        ...
        return \
            [
                ...
                {
                    "role": "user",
                    "content": f"Task: Find the cause behind the {subject}'s {problem}\nYour previous experience with similar tasks:\n{formated_prev_exp}Interaction History:\n{formated_history}available actions:{available_actions}\n{available_options}What action would you take next?\noutput format:\nreason: your reason\n$$$\n{output}"
...
class Chooser():
    ...
    def choose(self, history, subject, problem, choices, summary=None, posttest=False, prev_exp=None, summarizer=None):
        ...
        if summary is None:
            ...
            prompt = self.prompt_format.format_prompt(history, subject, problem, choices, summary, posttest=posttest,
                                                      prev_exp=prev_exp, summarizer=summarizer)
        for (i, p) in enumerate(prompt):
            length = len(p["content"].split())
            print(length)
...
class Recommender():
    ...
    def rec(self, history, subject, problem, valid_subjects, valid_topics, valid_causes, summary=None, posttest=False,
        ...
        if summary is None:
            prompt = self.prompt_format.format_prompt(history, subject, problem, valid_subjects, valid_topics,
                                                      valid_causes, posttest=posttest, prev_exp=prev_exp,
                                                      summarizer=summarizer)
        else:
            prompt = self.prompt_format.format_prompt(history, subject, problem, valid_subjects, valid_topics,
                                                      valid_causes, summary, posttest=posttest, prev_exp=prev_exp,
                                                      summarizer=summarizer)
        for (i, p) in enumerate(prompt):
            length = len(p["content"].split())
            print(length)
...
class Recommender_with_retry():
    ...
    def rec(self, history, subject, problem, valid_subjects, valid_topics, valid_causes, previous_suggestions,
        ...
        if summary is None:
            prompt = self.prompt_format.format_prompt(history, subject, problem, valid_subjects, valid_topics,
                                                      valid_causes, previous_suggestions=previous_suggestions,
                                                      posttest=posttest, prev_exp=prev_exp,
                                                      summarizer=summarizer)
        else:
            prompt = self.prompt_format.format_prompt(history, subject, problem, valid_subjects, valid_topics,
                                                      valid_causes, summary=summary,
...
class Player():
    ...
    def play(self, history, subject, problem, valid_subjects, valid_topics, valid_causes, summary=None, posttest=False,
        ...
        if summary is None:
            prompt = self.prompt_format.format_prompt(history, subject, problem, valid_subjects, valid_topics,
                                                      valid_causes, summarizer=summarizer, posttest=posttest,
                                                      prev_exp=prev_exp)
        else:
            prompt = self.prompt_format.format_prompt(history, subject, problem, valid_subjects, valid_topics,
                                                      valid_causes, summary, summarizer=summarizer, posttest=posttest,
                                                      prev_exp=prev_exp)
        for (i, p) in enumerate(prompt):
            length = len(p["content"].split())
            print(length)
...
If you think you have asked enough questions, please write "i want to propose the most probable diagnosis." as the next action. Remember that asking the same question again will not give you a different answer. Always follow the format of available actions. if the patient asks you what's to most probable cause behind their problem you can't continue asking questions make sure you recommend diagnoses then."""
