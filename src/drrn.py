...
class DRRNAgent:
    def __init__(self, args, state_dim):
        ...
        state_memories = []
        action_memories = []
        for x in os.listdir(args["memory_path"]):
            if "state" in x:
                state_memories.append(x)
            elif "action" in x:
                action_memories.append(x)
        state_memory_num = len(state_memories)
        action_memory_num = len(action_memories)
        ...
        self.state_encoder = Encoder(encoder_type=args["encoder_type"], emb_dim=args["embedding_dim"],
                                     ...
                                     memory_path=pjoin(args["memory_path"],
                                                       args["encoder_type"] + "_" + f"state_memory{state_memory_num}.json"))
        self.action_encoder = Encoder(encoder_type=args["encoder_type"], emb_dim=args["embedding_dim"],
                                      memory_size=args["encoder_memory_size"], save_memory=args["save_memory"],
                                      memory_path=pjoin(args["memory_path"],
                                                        args["encoder_type"] + "_"+f"action_memory{action_memory_num}.json"))
